...
/*
 * @Description: 解析下发数据的队列逻辑处理，接收手机发送的数据
 * @param: null
 * @return:
 */
...
static void gatts_profile_a_event_handler(esp_gatts_cb_event_t event,
  ...
  case ESP_GATTS_WRITE_EVT: {
    if (!param->write.is_prep) {
      ...
      sprintf(user_data.allData, "{\"red\":%d,\"green\":%d,\"blue\":%d}",
...
void app_main(void) {
  ...
  sprintf(deviceUUID, "%02x%02x%02x%02x%02x%02x", mac[0], mac[1], mac[2],
          mac[3], mac[4], mac[5]);
  ...
  if (mHandlerParseJSON == NULL) {
    xTaskCreate(Task_ParseJSON, "Task_ParseJSON", 1024 * 3, NULL, 3,
                &mHandlerParseJSON);
  }
