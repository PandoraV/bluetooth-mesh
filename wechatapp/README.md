...
1. 目前功能要求最高的是由 `FileSystemManager.access` 所依赖的基础库版本为 2.19.2，低版本不会限制使用该小程序，只是效果上可能无法达到理想的结果

2. 目前在鸿蒙系统上在连接蓝牙时偶现不能查找蓝牙设备，消息提示形式以`toast`形式推送，错误代码为`10008`，此错误为微信官方提供蓝牙读写API的原生bug，只能等待一段时间后bug自行消失，或更换其他设备。此bug难以复现，微信官方至今未修复。

3. 小程序连接蓝牙时需要联网调取微信提供的包文件，并非小程序实时联网上传/下载数据；在非联网的状态下，蓝牙连接和读写可能存在问题，同样归因于微信官方。
...
- 设备状态实时状态页：展示设备当前监测数据，具备指令操作功能，提供导出全部接口

## 使用教程

1. 连接设备

在首次启动该程序时，程序会提示用户开启蓝牙权限，因为该程序需要使用蓝牙进行数据收发，因此需要用户对蓝牙进行授权。

在获得用户授权之后，程序将会搜索附近的蓝牙设备，如果设备列表中没有出现用户的设备，只需要确认设备开启后并在手机蓝牙接收范围之内，再次点击“搜索设备”按钮进行点击即可。选择对应的设备之后点击设备右边的“连接”按钮即可开始与设备进行连接匹配，匹配成功之后，将进入下一步。

2. 监听数据

连接成功之后，此时手机便开始不断地接受蓝牙设备发来的信息。从上到下显示的信息依次为：设备名称、设备 UUID、监听操作、接收到的数据、文本输入框、发送文本按钮、时间间隔输入框、发送调节时间间隔命令按钮、断开连接按钮、导出数据按钮。

接收到的数据将手机收到的蓝牙消息实时显示出来，测试中用“-1”代表蓝牙的监测设备异常（如未开启）。如果此时将“监听”的勾选框的选中状态切换为“未选中”（即点击“监听”），此时手机将暂停接收蓝牙设备发送的数据，再次点击“监听”即可恢复接收蓝牙设备发送的数据并将事实数据更新在用户面前。

3. 导出数据

在上面一节中介绍了该页面的整体布局，只需要点击底部的“导出数据”按钮，程序会将该设备的历史消息转化为 csv 格式的文件，用户可将该文件传输给好友、文件传输助手。下图演示的是导出到文件传输助手后用表格软件（如金山文档、记事本、Excel）查看数据的流程和结果。

如果需要删除该小程序所存储的数据，只需要在微信页面下拉，在小程序列表中长摁该小程序进行删除即可。

4. 发送指令

在第二小节中介绍了该页面的整体布局，只需要在有“纯文本信息”提示的输入框输入对应的文字，点击“发送文本”，手机将会将用户输入的文本发送到蓝牙设备上。

如果要调节采样时间间隔的话，只需要在有“调整时间间隔”提示的输入框输入对应的时间间隔，点击“发送命令”之后手机将会告诉蓝牙设备要调节数据发送的时间间隔，蓝牙设备收到之后将会进行后续操作。

5. 切换设备

点击底部的“断开连接”按钮，或者左滑返回设备搜索页面即可。然后在设备列表里重新选择设备即可。
...
前几版代码中，将气体浓度加和之后乘`0.1`会频繁出现舍入误差。